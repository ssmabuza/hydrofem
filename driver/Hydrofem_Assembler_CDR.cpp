...
#include "Hydrofem_Assembler_CDR.hpp"
                         ...
                         const double beta) const
{
  ...
  const Problem_CDR::Ptr problem = std::dynamic_pointer_cast<Problem_CDR>(m_problem); assert(problem);
  const BC_CDR::Ptr bc = std::dynamic_pointer_cast<BC_CDR>(problem->getBoundaryCondition()); assert(bc);
  ...
  // const double L     = problem->L();
  // const double v_max = problem->v_max();
  const double diff  = problem->diff();

  // fluid velocity profile
  const std::function<SPoint(SPoint)> vel = bc->getFluidVelocity();
  ...
  for (int elem_ind = 0; elem_ind < m_dofmapper->nelements(); ++elem_ind)
  {
    ...
    const auto& q_weights = quad_e.get_q_weights();  // const double  // const double H     = problem->H();
  // const double L     = problem->L();
  // const double v_max = problem->v_max(); H     = problem->H();
  // const double L     = problem->L();
  // const double v_max = problem->v_max();
    ...
    for (std::size_t dof_i = 0; dof_i < loc_ind.size(); ++dof_i)
    {
      ...
      for (std::size_t dof_j = 0; dof_j <= dof_i; ++dof_j)
      {
        ...
        for (int quad_ind = 0; quad_ind < quad_e.size(); ++quad_ind)
        {
          ...
          val += q_weights.at(quad_ind) * phi_i * phi_j;
        }
    ...
    for (std::size_t dof_i = 0; dof_i < loc_ind.size(); ++dof_i)
    {
      ...
      for (std::size_t dof_j = 0; dof_j <= dof_i; ++dof_j)
      {
        ...
        for (int quad_ind = 0; quad_ind < quad_e.size(); ++quad_ind)
        {
          ...
          val += q_weights.at(quad_ind) * diff *(dphi_i.x() * dphi_j.x() + dphi_i.y() * dphi_j.y());
        }
  ...
  //   {
  //     throw std::runtime_error("AFC contribution in \"Assembler_CDR\" is not conservative.");
  //   }
