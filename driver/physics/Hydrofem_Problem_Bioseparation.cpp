...
double constExpression(const double /*x*/, const double /*y*/) { return 1.0; }

class ConstExpr : public ScalarAnalyticalExpression
{
public:

  ConstExpr() { m_is_transient = true; }

  //! \brief can be used for exact solution/ boundary values
  virtual double evaluate(const SPoint& x)
  { assert(!m_is_transient); return constExpression(x.x(),x.y()); }
...
void Problem_Bioseparation::init()
{
  ...
  m_u_in = std::make_shared<ConstExpr>();
...
void Problem_Bioseparation::addOptionsCallback(po::options_description &config)
{
  ...
  config.add_options()
    ...
    ("xf",po::value<double>(&m_xf)->default_value(1.6),"Right value for x.")
